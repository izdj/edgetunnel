...
let userID = 'ad7835cc-d239-4d0d-a076-d6e8958cc713';
let kvUUID;
...
let proxyPort = 443;
let proxyIpTxt = atob('aHR0cHM6Ly9yYXcuZ2l0aHVidXNlcmNvbnRlbnQuY29tL3lvbmdnZWtray9DbG91ZGZsYXJlX3ZsZXNzX3Ryb2phbi9tYWluL1Byb3h5SVAudHh0');
...
let ipUrlTxt = [
	atob('aHR0cHM6Ly9yYXcuZ2l0aHVidXNlcmNvbnRlbnQuY29tL3lvbmdnZWtray9DbG91ZGZsYXJlX3ZsZXNzX3Ryb2phbi9tYWluL1Byb3h5SVAudHh0')
];
let ipUrlCsv = [
	// atob('aHR0cHM6Ly9yYXcuZ2l0aHVidXNlcmNvbnRlbnQuY29tL3lvbmdnZWtray9DbG91ZGZsYXJlX3ZsZXNzX3Ryb2phbi9tYWluL2lwdjQuY3N2')
];
// Preferred addresses with optional TLS subscription
let ipLocal = [
	'visa.cn:443#youtube.com/@ACL4SSR',
	'icook.hk#t.me/ACL4SSR',
	'time.is#github.com/ACL4SSR'
];
let noTLS = 'false';
let sl = 5;

let tagName = atob('Y2Zrdg==');
...
let subConfig = "https://raw.githubusercontent.com/ACL4SSR/ACL4SSR/master/Clash/config/ACL4SSR_Online_Full_MultiMode.ini"; // Subscription profile
let fileName = 'cf';
let isBase64 = true;
...
let projectName = atob('Y2Zrdi9jZi1rdi10dW5uZWw=');
let ytName = atob('aHR0cHM6Ly93d3cueW91dHViZS5jb20vQGNsb3VkZmxhcmU=');
...
// const MY_KV_NAMESPACE = atob('Y2Zrdg==');
const MY_KV_UUID_KEY = atob('VVVJRA==');;

async function checkKVNamespaceBinding(env) {
	if (typeof env.cfkv === 'undefined') {
		return new Response('Error: cfkv KV_NAMESPACE is not bound.', {
			status: 400,
		})
	}
}
...
async function getKVData(env) {
	const value = await env.cfkv.get(MY_KV_UUID_KEY);
...
async function setKVData(request, env) {
	...
	try {
		await env.cfkv.put(MY_KV_UUID_KEY, value);

		// 读取存入的值，确认是否成功
		const storedValue = await env.cfkv.get(MY_KV_UUID_KEY);
...
async function nginx() {
	...
	<head>
	<meta charset="UTF-8">
	<title>Welcome to nginx! 301</title>
	<style>
		body {
			width: 35em;
			margin: 0 auto;
